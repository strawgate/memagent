...
fn compliance_empty_lines() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        assert_eq!(get_int(batch, "seq$int", 0), Some(0));
        assert_eq!(get_int(batch, "seq$int", 1), Some(1));
        assert_eq!(get_int(batch, "seq$int", 2), Some(2));
    });
}
...
fn compliance_huge_line() {
    ...
    assert_both_scanners(input.as_bytes(), |batch| {
        ...
        let val = get_str(batch, "msg$str", 0).expect("msg$str should exist");
...
fn compliance_nested_json_value() {
    ...
    assert_both_scanners(&input_nl, |batch| {
        assert_eq!(batch.num_rows(), 1);
        let val = get_str(batch, "data$str", 0).expect("data$str should exist");
        assert!(
...
fn compliance_array_value() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 1);
        let val = get_str(batch, "tags$str", 0).expect("tags$str should exist");
...
fn compliance_duplicate_keys() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 1);
        let val = get_str(batch, "a$str", 0).expect("a$str should exist");
...
fn compliance_type_conflict() {
    ...
    // Both $int and $str columns should exist, with nulls where type doesn't match.
    let input = b"{\"status\":200}\n{\"status\":\"OK\"}\n";
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 2);

        // Row 0: status$int=200, status$str=null
        assert_eq!(get_int(batch, "status$int", 0), Some(200));
        assert_null(batch, "status$str", 0);

        // Row 1: status$int=null, status$str="OK"
        assert_null(batch, "status$int", 1);
        assert_eq!(get_str(batch, "status$str", 1), Some("OK".to_string()));
    });
}
...
fn compliance_integer_boundaries() {
    ...
    assert_both_scanners(input.as_bytes(), |batch| {
        ...
        assert_eq!(get_int(batch, "n$int", 0), Some(i64::MAX));

        // Row 1: i64::MIN as int
        assert_eq!(get_int(batch, "n$int", 1), Some(i64::MIN));

        // Row 2: overflow -> float column
        assert_null(batch, "n$int", 2);
        let f = get_float(batch, "n$float", 2).expect("n$float should exist for overflow");
...
fn compliance_float_precision() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        let v0 = get_float(batch, "f$float", 0).expect("pi");
        assert!(
            (v0 - std::f64::consts::PI).abs() < 1e-15,
            "pi mismatch: {v0}"
        );

        let v1 = get_float(batch, "f$float", 1).expect("1e308");
        assert!((v1 - 1e308).abs() < 1e292, "1e308 mismatch: {v1}");

        let v2 = get_float(batch, "f$float", 2).expect("-1e308");
        assert!((v2 - (-1e308)).abs() < 1e292, "-1e308 mismatch: {v2}");

        let v3 = get_float(batch, "f$float", 3).expect("5e-324");
...
fn compliance_boolean_as_string() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 2);
        assert_eq!(get_str(batch, "flag$str", 0), Some("true".to_string()));
        assert_eq!(get_str(batch, "flag$str", 1), Some("false".to_string()));
    });
}
...
fn compliance_null_value() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 1);
        assert_not_null(batch, "a$str", 0);
        assert_eq!(get_str(batch, "a$str", 0), Some("x".to_string()));
        // b is null — it may not have a column at all, or the column is null.
        assert_null(batch, "b$str", 0);
        assert_null(batch, "b$int", 0);
        assert_null(batch, "b$float", 0);
        assert_not_null(batch, "c$str", 0);
        assert_eq!(get_str(batch, "c$str", 0), Some("y".to_string()));
    });
}
...
fn compliance_unicode_keys_and_values() {
    ...
    assert_both_scanners(input.as_bytes(), |batch| {
        ...
        let name = get_str(batch, "名前$str", 0).expect("CJK key should exist");
        assert_eq!(name, "太郎");
        let emoji = get_str(batch, "emoji$str", 0).expect("emoji key should exist");
        assert_eq!(emoji, "🎉");
    });
}
...
fn compliance_empty_string_value() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        assert_not_null(batch, "key$str", 0);
        let val = get_str(batch, "key$str", 0).expect("key$str should exist");
...
fn compliance_whitespace_variations() {
    ...
    assert_eq!(
        get_str(&batch_compact, "a$str", 0),
        get_str(&batch_spaced, "a$str", 0),
    );
    assert_eq!(
        get_int(&batch_compact, "c$int", 0),
        get_int(&batch_spaced, "c$int", 0),
    );
    ...
    assert_eq!(
        get_str(&batch_compact_s, "a$str", 0),
        get_str(&batch_spaced_s, "a$str", 0),
    );
}
...
fn compliance_non_object_lines() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        assert_eq!(get_str(batch, "valid$str", 5), Some("object".to_string()));
        // Rows 0-4 should have null for the valid$str column.
        for row in 0..5 {
            assert_null(batch, "valid$str", row);
        }
    });
}
...
fn compliance_trailing_no_newline() {
    ...
    // (scan$into uses eol=len when no \n is found and processes if pos < eol).
    let input = b"{\"a\":\"complete\"}\n{\"b\":\"no newline at end\"}";
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 2);
        assert_eq!(get_str(batch, "a$str", 0), Some("complete".to_string()));
        assert_eq!(
            get_str(batch, "b$str", 1),
...
fn compliance_cri_format() {
    ...
    assert_eq!(batch.num_rows(), 1);
    assert_eq!(get_str(&batch, "msg$str", 0), Some("hello".to_string()));
}
...
fn compliance_numeric_string_not_coerced() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 1);
        assert_eq!(get_str(batch, "port$str", 0), Some("8080".to_string()));
        // Should NOT appear as an int column.
        assert!(
            batch.column_by_name("port$int").is_none(),
...
fn compliance_mixed_type_across_rows() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        // Row 0: int
        assert_eq!(get_int(batch, "v$int", 0), Some(42));

        // Row 1: float
        let f = get_float(batch, "v$float", 1).expect("v$float row 1");
        #[allow(clippy::approx_constant)]
        let expected = 3.14;
        assert!((f - expected).abs() < 1e-10);

        // Row 2: string "text"
        assert_eq!(get_str(batch, "v$str", 2), Some("text".to_string()));

        // Row 3: bool stored as string
        assert_eq!(get_str(batch, "v$str", 3), Some("true".to_string()));

        // Row 4: null — all typed columns should be null for this row.
        assert_null(batch, "v$int", 4);
        assert_null(batch, "v$float", 4);
        assert_null(batch, "v$str", 4);
    });
}

#[test]
fn compliance_negative_zero_integer() {
    // -0 as an integer: parse$int_fast returns Some(0).
    let input = b"{\"v\":-0}\n";
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 1);
        assert_eq!(get_int(batch, "v$int", 0), Some(0));
    });
}
...
fn compliance_batch_reuse_isolation() {
    ...
    if b2.column_by_name("a$str").is_some() {
        assert_null(&b2, "a$str", 0);
    }
    assert_eq!(get_str(&b2, "b$str", 0), Some("second".to_string()));
}
...
fn compliance_json_with_carriage_return() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        assert_eq!(batch.num_rows(), 2);
        assert_eq!(get_int(batch, "a$int", 0), Some(1));
        assert_eq!(get_int(batch, "b$int", 1), Some(2));
    });
}
...
fn compliance_nested_array_of_objects() {
    ...
    assert_both_scanners(&input_nl, |batch| {
        assert_eq!(batch.num_rows(), 1);
        let val = get_str(batch, "items$str", 0).expect("items$str");
        assert!(val.starts_with('['));
        assert!(val.ends_with(']'));
...
fn compliance_empty_nested_structures() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 1);
        assert_eq!(get_str(batch, "obj$str", 0), Some("{}".to_string()));
        assert_eq!(get_str(batch, "arr$str", 0), Some("[]".to_string()));
    });
}
...
fn compliance_scientific_notation_integers() {
    ...
    assert_both_scanners(input, |batch| {
        ...
        let f = get_float(batch, "v$float", 0).expect("v$float");
...
fn compliance_zero_as_all_types() {
    ...
    assert_both_scanners(input, |batch| {
        assert_eq!(batch.num_rows(), 3);
        assert_eq!(get_int(batch, "a$int", 0), Some(0));
        assert_eq!(get_float(batch, "a$float", 1), Some(0.0));
        assert_eq!(get_str(batch, "a$str", 2), Some("0".to_string()));
    });
}
...
fn compliance_field_ordering_preserved() {
    ...
    assert_eq!(names[0], "z$int");
    assert_eq!(names[1], "a$int");
    assert_eq!(names[2], "m$int");
}
...
fn compliance_braces_inside_string_values() {
    ...
    assert_both_scanners(&input_nl, |batch| {
        assert_eq!(batch.num_rows(), 1);
        assert_eq!(get_int(batch, "count$int", 0), Some(1));
    });
}
...
fn compliance_large_batch_row_count() {
    ...
    assert_both_scanners(&buf, |batch| {
        ...
        assert_eq!(get_int(batch, "i$int", 0), Some(0));
        assert_eq!(get_int(batch, "i$int", 9_999), Some(9_999));
    });
}

...
fn verify_roundtrip(original: &RecordBatch, roundtrip: &[RecordBatch]) {
    let total_rows: usize = roundtrip.iter().map(arrow::array::RecordBatch::num_rows).sum();
    assert_eq!(
        original.num_rows(),
        total_rows,
...
fn main() {
    ...
    println!(
        "done ({} batches, {} rows)",
        rt_uncmp.len(),
        rt_uncmp.iter().map(arrow::array::RecordBatch::num_rows).sum::<usize>()
    );
    ...
    println!(
        "done ({} batches, {} rows)",
        rt_zstd.len(),
        rt_zstd.iter().map(arrow::array::RecordBatch::num_rows).sum::<usize>()
    );
    ...
    println!(
        "done ({} batches, {} rows)",
        rt_mmap.len(),
        rt_mmap.iter().map(arrow::array::RecordBatch::num_rows).sum::<usize>()
    );

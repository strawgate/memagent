...
//!
//! Uses `validate_utf8: false` so arbitrary byte sequences are accepted;
//! this is intentional — the target checks builder consistency, not UTF-8 safety.
...
fuzz_target!(|data: &[u8]| {
    ...
    let mut storage_scanner = SimdScanner::new(config.clone());
    let Ok(storage_batch) = storage_scanner.scan(data) else {
        return;
    };
    ...
    let mut streaming_scanner = StreamingSimdScanner::new(config2);
    let Ok(streaming_batch) = streaming_scanner.scan(bytes::Bytes::copy_from_slice(data)) else {
        return;
    };

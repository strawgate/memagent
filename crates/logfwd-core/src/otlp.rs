...
mod verification {
    ...
    /// Prove encode_varint + decode_varint form a roundtrip for all u64 values.
    ///
    /// This is the key correctness proof for the varint codec: whatever is
    /// encoded must decode to the original value. Without this proof, the
    /// existing encode-only proofs cannot guarantee the wire format is usable
    /// by a decoder.
    #[kani::proof]
    #[kani::unwind(12)] // encode: max 10 bytes; decode: max 10 iterations + 2 overhead
    #[kani::solver(kissat)] // byte-level bit ops: kissat consistently fastest
    fn verify_decode_varint_roundtrip() {
        let value: u64 = kani::any();

        let mut buf = alloc::vec::Vec::with_capacity(10);
        encode_varint(&mut buf, value);

        // Decode must succeed (encode always produces valid varint bytes).
        let result = decode_varint(&buf, 0);
        assert!(
            result.is_ok(),
            "decode_varint must accept encode_varint output"
        );

        let (decoded, consumed) = result.unwrap();
        assert!(decoded == value, "roundtrip mismatch");
        assert!(
            consumed == buf.len(),
            "decoded length must match encoded length"
        );
    ...
    /// Crash-freedom proof for decode_varint on arbitrary byte inputs.
    ///
    /// Guarantees that no byte sequence of ≤12 bytes causes a panic —
    /// the only failure mode is returning `Err(&str)`, never a panic from
    /// OOB indexing or arithmetic overflow.
    ///
    /// Note: The existing `verify_varint_format_and_roundtrip` only runs
    /// the encoder; this proof runs the DECODER on adversarial inputs.
    #[kani::proof]
    #[kani::unwind(14)] // decode loop: up to 12 bytes, +2 for exit checks
    #[kani::solver(kissat)]
    fn verify_decode_varint_no_panic() {
        // Crash-freedom proof: decode_varint must not panic on any input.
        const N: usize = 12;
    ...
    /// Prove decode_tag correctly decodes what encode_tag encodes.
    ///
    /// Covers the (field_number, wire_type) → tag varint → (field_number,
    /// wire_type) roundtrip. Any mismatch here would silently misroute fields
    /// during protobuf parsing.
    #[kani::proof]
    #[kani::unwind(12)]
    #[kani::solver(kissat)]
    fn verify_decode_tag_roundtrip() {
        let field_number: u32 = kani::any();
        let wire_type: u8 = kani::any();

        kani::assume(field_number >= 1);
        kani::assume(field_number <= 0x1FFFFFFF); // max valid protobuf field number
        kani::assume(wire_type <= 5); // valid wire types: 0, 1, 2, 5

        let mut buf = alloc::vec::Vec::with_capacity(10);
        encode_tag(&mut buf, field_number, wire_type);

        let result = decode_tag(&buf, 0);
        assert!(result.is_ok(), "decode_tag must accept encode_tag output");

        let (dec_field, dec_wire, consumed) = result.unwrap();
        assert!(dec_field == field_number, "field_number roundtrip mismatch");
        assert!(dec_wire == wire_type, "wire_type roundtrip mismatch");
        assert!(
            consumed == buf.len(),
            "consumed bytes must match encoded tag size"
        );

        kani::cover!(field_number == 1 && wire_type == 0, "smallest valid tag");
        kani::cover!(
            field_number == 0x1FFFFFFF && wire_type == 5,
            "largest valid tag"
        );
        kani::cover!(
            field_number == 16 && wire_type == 2,
            "typical len-delimited field"
        );
    }
}

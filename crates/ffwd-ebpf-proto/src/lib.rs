...
//!
//! ```text
//!    Container              Kernel (eBPF)           Userspace (ffwd)
//!    ─────────              ──────────────           ──────────────────
//!    write(1, data, n)
//!         │
//!         ▼
...
//!    CRI log file
//!    (still happens, but
//!     ffwd doesn't read it)
//! ```
//!
//! # Integration with ffwd
//!
//! `EbpfInput` implements the `InputSource` trait from ffwd-core.
...
/// |--------|-------------|-------------------|
/// | Latency (write → ffwd) | 15-300ms | 1-5ms |

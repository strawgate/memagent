...
use ffwd_arrow::conflict_schema::normalize_conflict_columns;
use ffwd_core::otlp::{
...
pub(super) use ffwd_core::otlp::{encode_fixed32, encode_tag, encode_varint};
    ...
    fn encode_batch_with_row_encoder(
        ...
        for (key, scope_key, record_ranges) in &grouped_ranges {
            let scope_name = scope_key
                .0
                .unwrap_or_else(|| std::str::from_utf8(SCOPE_NAME).unwrap_or("ffwd"));

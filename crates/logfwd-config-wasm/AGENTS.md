...
Exposes a small JavaScript API backed by the same `logfwd-config` Rust types used at runtime, so users can generate and validate `logfwd.yaml` configs in the browser without a server.
...
```bash
wasm-pack build crates/logfwd-config-wasm --target web --out-dir ../../book/public/wasm/logfwd-config
```

Output lands in `book/public/wasm/logfwd-config/`. The `.js` and `.wasm` files are committed there.

...
//!   ES_INDEX      — target index base name (default: ffwd-bench)
//!
//! Usage:
...
fn es_index() -> String {
    std::env::var("ES_INDEX").unwrap_or_else(|_| "ffwd-bench".to_string())
}
...
fn run_worker(
    ...
    while Instant::now() < deadline {
        ...
        match rt.block_on(sink.send_batch(&result, &meta)) {
            ffwd_output::sink::SendResult::Ok => {
            ...
            ffwd_output::sink::SendResult::IoError(e) => {
                eprintln!("[worker {worker_id}] send_batch error: {e}");
                total_errors.fetch_add(1, Ordering::Relaxed);
            }
            ffwd_output::sink::SendResult::RetryAfter(delay) => {
                eprintln!("[worker {worker_id}] send_batch asked to retry after {delay:?}");
                total_errors.fetch_add(1, Ordering::Relaxed);
            }
            ffwd_output::sink::SendResult::Rejected(reason) => {

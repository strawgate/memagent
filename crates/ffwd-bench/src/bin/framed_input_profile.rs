...
fn main() {
    ...
    println!(
        "- `crates/ffwd-io/src/framed.rs` — `remainder` + `extend_from_slice(&bytes)` copies every new chunk into a fresh/retaken `Vec<u8>`, even when the remainder is empty."
    );
    println!(
...
fn print_help() {
    println!("Usage: cargo run -p ffwd-bench --release --bin framed_input_profile -- [OPTIONS]");

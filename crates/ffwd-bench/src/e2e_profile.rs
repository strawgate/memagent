...
//! Run with: cargo run -p ffwd-bench --release --features bench-tools --bin e2e_profile
...
fn main() {
    ...
    let t3 = Instant::now();
    let mut otlp_sink = ffwd_output::OtlpSink::new(
        "bench".to_string(),
        "http://localhost:19877".to_string(),
        ffwd_output::OtlpProtocol::Http,
        ffwd_output::Compression::None,
        vec![],
        reqwest::Client::new(),
    ...
    for n in [100_000, 500_000, 1_000_000] {
        ...
        let t = Instant::now();
        let mut sink = ffwd_output::OtlpSink::new(
            "b".into(),
            "http://x".into(),
            ffwd_output::OtlpProtocol::Http,
            ffwd_output::Compression::None,

...
```rust
use ffwd_lint_attrs::{hot_path, cancel_safe, no_panic, pure, owned_by_actor};
...
3. Implement `LateLintPass::check_fn` or similar. If the lint relies
   on a `#[ffwd_lint_attrs::<name>]` attribute, look for the marker
   `__ffwd_<name>__` doc attribute via `util::has_doc_marker`.
4. Register in `src/lib.rs::register_lints`.
5. Add a corresponding attribute in `crates/ffwd-lint-attrs/src/lib.rs`.
...
7. Document in the Lints table above, in
   ...
   `crates/ffwd-lint-attrs/src/lib.rs` crate-level docs.

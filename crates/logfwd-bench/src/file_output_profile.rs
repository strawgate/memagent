...
fn main() {
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        eprintln!(
            "  --output DIR                Output directory (default: /tmp/logfwd-file-profile)"
        );
        eprintln!();
    ...
    let output_dir = parse_flag(&args, "--output", "/tmp/logfwd-file-profile");
    ...
    match mode.as_str() {
        ...
        "alloc" => {
            ...
            #[cfg(not(feature = "otlp-profile-alloc"))]
            {
                ...
                eprintln!(
                    "Run: cargo run -p logfwd-bench --release --features otlp-profile-alloc --bin file_output_profile -- --mode alloc"
                );
                std::process::exit(1);
            }
        }

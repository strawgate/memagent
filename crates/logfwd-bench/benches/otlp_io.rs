...
fn build_fixture(profile: FixtureProfile) -> FixtureData {
    ...
    if !profile.has_complex_any {
        ...
        let view_batch =
            decode_protobuf_bytes_to_batch_projected_view_experimental(payload_bytes.clone())
        ...
        assert_batch_matches(&batch, &detached_view_batch, profile.name);
...
fn bench_decode_materialize(c: &mut Criterion) {
    ...
    for fixture in &fixtures {
        ...
        group.throughput(Throughput::Elements(fixture.profile.total_rows() as u64));
        group.bench_with_input(
            BenchmarkId::new("prost_to_batch", fixture.profile.name),
            &fixture.payload,
            |b, payload| {
                b.iter(|| {
                    let batch = decode_protobuf_to_batch_prost_reference(payload)
        ...
        if !fixture.profile.has_complex_any {
            group.bench_with_input(
                ...
                |b, payload| {
                    b.iter(|| {
                        let batch = decode_protobuf_to_batch_projected_experimental(payload)
            ...
            group.bench_with_input(
                ...
                |b, payload| {
                    b.iter(|| {
                        let batch = decode_protobuf_bytes_to_batch_projected_view_experimental(
                            payload.clone(),
                        )
    ...
    for fixture in &fixtures {
        ...
        group.bench_with_input(
            ...
            |b, fixture| {
                ...
                b.iter(|| {
                    sink.encode_batch(&fixture.batch, &metadata);
        ...
        group.bench_with_input(
            ...
            |b, fixture| {
                let mut sink = make_otlp_sink(Compression::None);
                b.iter(|| {
                    sink.encode_batch_generated_fast(&fixture.batch, &metadata);
                    std::hint::black_box(sink.encoded_payload().len());
                });
            },
        );

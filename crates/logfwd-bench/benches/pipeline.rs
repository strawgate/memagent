...
fn bench_transform(c: &mut Criterion) {
    ...
    group.bench_function("projection_computed", |b| {
        let mut transform = SqlTransform::new(
            "SELECT level$str, message$str, status$int, duration_ms$int FROM logs",
        )
        .unwrap();
        b.iter(|| transform.execute_blocking(batch.clone()).unwrap())
    });
    ...
    group.bench_function("regexp_extract", |b| {
        let mut transform = SqlTransform::new(
            "SELECT regexp_extract(message$str, '(GET|POST) (\\S+)', 1) AS method, \
             regexp_extract(message$str, '(GET|POST) (\\S+)', 2) AS path FROM logs",
        )
        .unwrap();
        b.iter(|| transform.execute_blocking(batch.clone()).unwrap())
    });
    ...
    group.bench_function("grok_parse", |b| {
        let mut transform = SqlTransform::new(
            "SELECT grok(message$str, '%{WORD:method} %{URIPATH:path} %{WORD:proto}') AS parsed FROM logs",
        )
        .unwrap();
        b.iter(|| transform.execute_blocking(batch.clone()).unwrap())
    });
...
fn bench_end_to_end(c: &mut Criterion) {
    ...
    group.bench_function("scan_grok_filter_capture", |b| {
        let mut transform = SqlTransform::new(
            "SELECT grok(message$str, '%{WORD:method} %{URIPATH:path} %{WORD:proto}') AS parsed \
             FROM logs WHERE level$str != 'DEBUG'",
        )
        .unwrap();
        let mut sink = NullSink;
        b.iter(|| {

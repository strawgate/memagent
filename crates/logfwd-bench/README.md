...
`otlp_io.rs` keeps fixture synthesis, protobuf encoding, and prost/parity validation in setup only. The timed regions measure just the named stage, so:
...
The suite also includes an experimental wire-to-Arrow decoder for primitive fixtures; it is validated against the prost path before any benchmark timing starts.

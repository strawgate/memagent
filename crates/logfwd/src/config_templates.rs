...
fn strip_root_key<'a>(snippet: &'a str, root_key: &str) -> &'a str {
    snippet
...
fn push_pipeline_mapping_list_item(out: &mut String, snippet: &str, root_key: &str) {
    let body = strip_root_key(snippet.trim_end(), root_key);
    let mut lines = body.lines().filter(|line| !line.trim().is_empty());

    if let Some(first) = lines.next() {
        let _ = writeln!(out, "    {}s:", root_key.trim_end_matches(':'));
...
pub(crate) fn render_config(
    ...
    push_pipeline_mapping_list_item(&mut out, input.snippet, "input:");
    ...
    push_pipeline_mapping_list_item(&mut out, output.snippet, "output:");
...
pub(crate) fn render_use_case(use_case: &UseCaseTemplate, sql: &str) -> String {
    ...
    out.push('\n');
    out.push_str("pipelines:\n  default:\n");
    push_pipeline_mapping_list_item(&mut out, use_case.input, "input:");
    out.push_str("    transform: |\n");
    for line in sql.lines() {
        let _ = writeln!(out, "      {line}");
    }
    push_pipeline_mapping_list_item(&mut out, use_case.output, "output:");

...
"#,
    ...
    (stats.bytes_allocated as u64, rows as usize, batches as usize)
}
...
fn allocation_per_row_decreases_with_scale() {
    ...
    eprintln!("  10K rows:  {per_row_10k:.0} bytes/row  ({batches_10k} batches, {rows_10k} rows processed)");
    eprintln!("  100K rows: {per_row_100k:.0} bytes/row  ({batches_100k} batches, {rows_100k} rows processed)");
    eprintln!("  500K rows: {per_row_500k:.0} bytes/row  ({batches_500k} batches, {rows_500k} rows processed)");
    eprintln!("--------------------------");

    // Per-row cost should decrease as fixed overhead is amortized.
    // At minimum, 100K should be <= 10K (fixed costs amortized over more rows).
    assert!(
        per_row_100k <= per_row_10k * 1.1,

// segment.rs — Checkpoint segment file format, writer, reader, and recovery.
//
...
impl SegmentFile {
    ...
    pub fn open(path: &Path, expected_sql_hash: Option<u64>) -> SegmentStatus {
        ...
        let footer = match SegmentFooter::from_bytes(&footer_buf) {
            Ok(f) => f,
            Err(_) => return SegmentStatus::Incomplete(path.to_path_buf()),
        };
        ...
        if let Some(expected) = expected_sql_hash {
            if header.sql_hash != expected && header.sql_hash != 0 {

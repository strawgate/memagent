...
/// Convert ffwd's flat `RecordBatch` to OTAP star schema.
///
...
///    through LOG_ATTRS instead), or a default `scope_name="ffwd"` row otherwise.
...
pub fn flat_to_star(batch: &RecordBatch) -> Result<StarSchema, ArrowError> {
    ...
    // Only emit the hardcoded "ffwd" default when no scope columns are present.
...
/// Build the single-row SCOPE_ATTRS table with scope_name="ffwd".

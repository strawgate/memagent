...
mod tests {
    ...
    fn test_float_udf() {
        ...
        let col = result
            ...
            .unwrap();
        assert!((col.value(0) - std::f64::consts::PI).abs() < 0.01);
        assert!(col.is_null(1));
        assert!((col.value(2) - std::f64::consts::E).abs() < 0.01);
    }
    ...
    fn test_enrichment_cross_join() {
        ...
        let _ = transform.add_enrichment_table(env_table);
    ...
    fn test_enrichment_unused_table_no_error() {
        ...
        let mut transform = SqlTransform::new("SELECT * FROM logs").unwrap();
        let _ = transform.add_enrichment_table(table);
    ...
    fn test_enrichment_empty_table_skipped() {
        ...
        let mut transform = SqlTransform::new("SELECT * FROM logs").unwrap();
        let _ = transform.add_enrichment_table(k8s);

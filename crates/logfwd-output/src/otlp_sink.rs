...
mod tests {
    ...
    #[derive(Debug, Clone, PartialEq, Eq)]
    struct OracleLogRecord {
        time_unix_nano: u64,
        observed_time_unix_nano: u64,
        severity_number: i32,
        severity_text: String,
        body: OracleValue,
        attributes: BTreeMap<String, OracleValue>,
        trace_id: Vec<u8>,
        span_id: Vec<u8>,
        flags: u32,
    }
    ...
    struct OracleRequest {
        resource_attributes: BTreeMap<String, OracleValue>,
        scope_name: String,
        scope_version: String,
        records: Vec<OracleLogRecord>,
    }

    fn oracle_value_from_any(
        value: &opentelemetry_proto::tonic::common::v1::AnyValue,
    ) -> OracleValue {
        use opentelemetry_proto::tonic::common::v1::any_value::Value;

        match &value.value {
            Some(Value::StringValue(s)) => OracleValue::String(s.clone()),
            Some(Value::IntValue(i)) => OracleValue::Int(*i),
            Some(Value::DoubleValue(d)) => OracleValue::DoubleBits(d.to_bits()),
            Some(Value::BoolValue(b)) => OracleValue::Bool(*b),
            Some(Value::BytesValue(bytes)) => OracleValue::Bytes(bytes.clone()),
            _ => OracleValue::Empty,
        }
    }

    fn oracle_request_from_proto(
        ...
        let scope = scope_logs
            .scope
            .as_ref()
            .expect("oracle request must contain InstrumentationScope");
                ...
                resource
                    ...
                    .map(|kv| {
                        (
                            ...
                            kv.value
                                .as_ref()
                                .map_or(OracleValue::Empty, oracle_value_from_any),
                        )
                    })
                    .collect()
            })
    ...
    #[test]
    fn encode_batch_matches_official_proto_oracle() {
        ...
        let metadata = BatchMetadata {
            resource_attrs: Arc::new(vec![("k8s.pod.name".to_string(), "my-pod".to_string())]),
            observed_time_ns: 1_700_000_000_000_000_000,
        };

        let mut sink = make_sink();
        sink.encode_batch(&batch, &metadata);
        let actual = ExportLogsServiceRequest::decode(sink.encoder_buf.as_slice())
            .expect("official protobuf decoder must decode custom encoder output");

        let expected = ExportLogsServiceRequest {
            resource_logs: vec![ResourceLogs {
                resource: Some(Resource {
                    attributes: vec![KeyValue {
                        key: "k8s.pod.name".into(),
                        value: Some(AnyValue {
                            value: Some(Value::StringValue("my-pod".into())),
                        }),
                    }],
                    ..Default::default()
                }),
        ...
        assert_eq!(
            oracle_request_from_proto(&actual),
            oracle_request_from_proto(&expected),
            "custom OTLP encoder must match the official protobuf oracle semantically"
        );
    }

...
impl StdinInput {
    ...
    pub fn new(name: impl Into<String>) -> Self {
        ...
        if let Err(err) = thread::Builder::new()
            .name("ffwd-stdin-reader".to_owned())
            .spawn(move || {
                let stdin = io::stdin();
...
impl FileInput {
    ...
    pub fn new(
        ...
        paths: &[PathBuf],
        config: TailConfig,
        stats: std::sync::Arc<ffwd_types::diagnostics::ComponentStats>,
    ) -> io::Result<Self> {
...
impl InputSource for FileInput {
    fn poll(&mut self) -> io::Result<Vec<SourceEvent>> {
        Ok(tail_events_to_source_events(self.tailer.poll()?))
    }

    fn poll_shutdown(&mut self) -> io::Result<Vec<SourceEvent>> {
        Ok(tail_events_to_source_events(self.tailer.poll_shutdown()?))
    }
...
fn tail_events_to_source_events(tail_events: Vec<TailEvent>) -> Vec<SourceEvent> {

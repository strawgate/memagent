...
fn unique_temp_dir(prefix: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    std::env::temp_dir().join(format!("ffwd-{prefix}-{nanos}-{}", std::process::id()))
}
...
fn generator_attribute_scalar_types_are_preserved() {
    ...
    let attributes = match &input.type_config {
        ffwd_config::InputTypeConfig::Generator(generator) => generator
            .generator
            .as_ref()
    ...
    assert!(matches!(
        attributes["count"],
        ffwd_config::GeneratorAttributeValueConfig::Integer(7)
    ));
    assert!(matches!(
        attributes["enabled"],
        ffwd_config::GeneratorAttributeValueConfig::Bool(true)
    ));
    assert!(matches!(
        attributes["label"],
        ffwd_config::GeneratorAttributeValueConfig::String(ref value) if value == "7"
    ));
}
...
fn env_generator_attribute_values_remain_strings() {
    ...
    assert!(matches!(
        attributes["enabled"],
        ffwd_config::GeneratorAttributeValueConfig::String(ref value) if value == "true"
    ));
}
...
fn env_expansion_preserves_yaml_hash_content() {
    ...
    let path = match &input.type_config {
        ffwd_config::InputTypeConfig::File(file) => file.path.as_str(),
...
fn env_bool_string_is_parsed_by_typed_schema() {
    ...
    let strict_path = match &input.type_config {
        ffwd_config::InputTypeConfig::Http(http) => {
...
fn env_bool_string_is_parsed_by_shared_tls_schema() {
    ...
    let ffwd_config::OutputConfigV2::Otlp(output) = &config.pipelines["default"].outputs[0] else {

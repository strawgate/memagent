...
fn file_output_same_as_input_rejected() {
    let path = yaml_quote_path(std::env::temp_dir().join("ffwd-feedback-test.log"));
    let yaml = format!(
        r"
pipelines:
  looping:
...
fn file_output_different_from_input_allowed() {
    let input_path = yaml_quote_path(std::env::temp_dir().join("ffwd-input.log"));
    let output_path = yaml_quote_path(std::env::temp_dir().join("ffwd-output.log"));
    let yaml = format!(
        r"
pipelines:
  ok:
...
fn file_output_same_as_input_rejected_after_lexical_normalization() {
    ...
    let output_path = yaml_quote_path(temp_dir.join("ffwd-feedback").join(".").join("app.log"));
    let yaml = format!(
        r"
pipelines:
  looping:

...
///
/// Note: `ffwd-io::generator::parse_iso8601_to_epoch_ms` performs the same
/// validation plus epoch conversion.  We duplicate the range checks here because
/// `ffwd-config` cannot depend on `ffwd-io` (wrong crate-dependency direction).
...
pipelines:
  looping:
    inputs:
      - type: file
        path: /tmp/ffwd-feedback-test.log
    outputs:
      - type: file
        path: /tmp/ffwd-feedback-test.log
        format: json
...
pipelines:
  ok:
    inputs:
      - type: file
        path: /tmp/ffwd-input.log
    outputs:
      - type: file
        path: /tmp/ffwd-output.log
        format: json
";

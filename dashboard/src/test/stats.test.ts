...
describe("computeStats", () => {
  ...
  it("returns null when only in-progress traces exist", () => {
    const traces = [
      makeTr({ in_progress: true, lifecycle_state: "scan_in_progress" }),
      makeTr({ in_progress: true, lifecycle_state: "scan_in_progress" }),
      makeTr({ in_progress: true, lifecycle_state: "scan_in_progress" }),
    ];
...
describe("buildLanes", () => {
  ...
  describe("pending traces", () => {
    ...
    it("completed traces with worker_id < 0 are not added to pending or worker lanes", () => {
      // worker_id = -1 but completed → neither pending nor worker lane
      const done = makeTr({ worker_id: -1 });
    ...
    it("mixes worker and pending traces correctly", () => {
      ...
      const p = makeTr({ worker_id: -1, in_progress: true, lifecycle_state: "queued_for_output" });

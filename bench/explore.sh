...
QUERIES=(
    ...
    "SELECT *, CASE WHEN duration_ms$int > 200 THEN 'slow' ELSE 'fast' END as speed$str FROM logs"
...
run_bench "wide" 100000 "SELECT timestamp$str, level$str FROM logs" "$TMPDIR/wide_100k.jsonl"
run_bench "wide" 100000 "SELECT timestamp$str, level$str, message$str, duration_ms$int FROM logs" "$TMPDIR/wide_100k.jsonl"
run_bench "wide" 100000 "SELECT * FROM logs WHERE level$str = 'ERROR'" "$TMPDIR/wide_100k.jsonl"
echo ""
...
run_bench "simple" 100000 "SELECT level$str, COUNT(*) FROM logs GROUP BY level$str" "$TMPDIR/simple_100000.jsonl"
run_bench "simple" 100000 "SELECT level$str, AVG(duration_ms$int), MIN(duration_ms$int), MAX(duration_ms$int) FROM logs GROUP BY level$str" "$TMPDIR/simple_100000.jsonl"
run_bench "wide" 100000 "SELECT region$str, namespace$str, COUNT(*) FROM logs GROUP BY region$str, namespace$str" "$TMPDIR/wide_100k.jsonl"
run_bench "wide" 100000 "SELECT service$str, method$str, AVG(duration_ms$int), COUNT(*) FROM logs GROUP BY service$str, method$str" "$TMPDIR/wide_100k.jsonl"
echo ""

# --- Dimension 6: Heavy transforms ---
echo "=== Dimension 6: Heavy Transforms (100K lines) ==="
run_bench "simple" 100000 "SELECT *, duration_ms$int * 1000 as duration_us FROM logs" "$TMPDIR/simple_100000.jsonl"
run_bench "simple" 100000 "SELECT *, UPPER(level$str) as level_upper FROM logs" "$TMPDIR/simple_100000.jsonl"
run_bench "simple" 100000 "SELECT *, LENGTH(message$str) as msg_len FROM logs" "$TMPDIR/simple_100000.jsonl"
run_bench "simple" 100000 "SELECT *, CONCAT(level$str, ':', service$str) as tag FROM logs" "$TMPDIR/simple_100000.jsonl"
echo ""

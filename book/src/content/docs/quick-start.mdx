...
:::note
The Cargo package is named `ffwd`; the installed CLI binary is `ff`. Docker images are published to `ghcr.io/strawgate/fastforward`.
:::
...
<Tabs>
  ...
  <TabItem label="Docker" icon="docker">
    ...
    docker run --rm \
      ...
      -v $(pwd)/config.yaml:/etc/ffwd/config.yaml:ro \
      ghcr.io/strawgate/fastforward:latest run --config /etc/ffwd/config.yaml
    ```
...
:::note
FastForward examples and generated configs use the `pipelines` map shown above.
:::

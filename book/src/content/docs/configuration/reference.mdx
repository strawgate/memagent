...
Supported enrichment types: `k8s_path`, `host_info`, `static`, `geo_database`, `csv`, `jsonl`.
...
Exposes information about the machine running logfwd.
...
Looks up geographic metadata from a MaxMind-compatible `.mmdb` database file.

| Field | Type | Description |
|-------|------|-------------|
| `format` | string | Database format. Only `mmdb` is supported. |
| `path` | string | Path to the `.mmdb` file. |
| `refresh_interval` | integer | Optional interval in seconds to reload the database. |
...
Loads a CSV file as an in-memory lookup table.

| Field | Type | Description |
|-------|------|-------------|
| `table_name` | string | SQL table name to use in queries. |
| `path` | string | Path to the CSV file. The first row must be a header row. |
...
```sql
SELECT l.*, a.owner
FROM logs l JOIN assets a ON l.host = a.hostname
```

### `jsonl` enrichment

Loads a JSON Lines file as an in-memory lookup table. Each line must be a JSON object.
...
enrichment:
  - type: jsonl
    ...
    path: /etc/logfwd/ip_owners.jsonl
```

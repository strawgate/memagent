...
FastForward pipeline configs use a top-level `pipelines` map. Each named
...
Read macOS unified log entries by running `log stream`. This input is only
...
Read structured entries from the systemd journal using either the native `sd_journal`
...
Send newline-delimited JSON rows to an HTTP endpoint with optional request-body
compression and auth headers.

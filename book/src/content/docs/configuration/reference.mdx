...
| `_source_path` | string | Full path to the source log file (file inputs only). |
...
| `grok(input, pattern)` | `grok(utf8, utf8) → Struct` | Apply a Grok pattern to `input` and return a struct with one Utf8 field per named capture. Non-matching rows produce NULL struct fields. |

...
Read objects from AWS S3 or an S3-compatible endpoint. The S3 input can poll
...
When `source_metadata` is `ecs`, `beats`, `otel`, or `vector`, S3 exposes the
...
Reads the macOS Unified Log (`OSLog`) using a background `log stream --style ndjson` process. Yields structured JSON objects natively representing OSLog entries. Requires macOS 11+.

```yaml
inputs:
  - type: macos_log
    level: default     # Filter level (e.g. debug, info, default)
    subsystem: com.apple.authd # Optional subsystem predicate
    process: sudo      # Optional process predicate
```

| Field | Type | Required | Description |
|---|---|---|---|
| `level` | `string` | No | Maps to `log stream --level`. Filters events below this level. |
| `subsystem` | `string` | No | Predicate filter for a specific subsystem. |
| `process` | `string` | No | Predicate filter for a specific process (by name or PID). |
...
| `compression` | enum | No | `zstd`, `gzip`, or `none`. Invalid values are rejected while parsing config. |

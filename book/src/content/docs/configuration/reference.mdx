...
| `source_metadata` | boolean | No | Attach source metadata columns (`_source_id`, `_input`, `_source_path`) when SQL requests them. Defaults to `false`; set `true` before referencing those columns or using `k8s_path` joins. |
...
| `_source_id` | uint64 | Stable row-level source identity when an input can identify a sender/source. File inputs use file identity; UDP uses sender address identity. |
| `_input` | string | Configured input name for the row source. |
| `_source_path` | string | Source file path (file-backed inputs only). Used as a join key for `k8s_path` enrichment. |
...
scanner-ready chunks and, when `source_metadata: true` is set on the input,
materialized as table columns before SQL runs. For compatibility, `SELECT *`
includes `_source_path` for opted-in inputs. New metadata columns such as
`_source_id` and `_input` are included only when SQL references them explicitly.
...
Queries that join `logs._source_path` to this table must set
`source_metadata: true` on the file input. Source paths are never written into
raw log bytes.

...
The `endpoint` must include the full path (e.g. `/v1/logs` for HTTP). logfwd posts directly to this URL.
...
output:
  ...
  pipeline: parse_logs    # optional: Ingest Node pipeline name
  action: index           # index | create (optional, default index)
  timeout_sec: 30         # bulk request timeout (optional, default 30)
  max_bulk_bytes: 5242880 # max bulk payload size (optional, default 5 MiB)
...
| `pipeline` | string | No | — | Ingest Node pipeline name |
| `action` | string | No | `index` | Bulk API action (`index` or `create`) |
| `timeout_sec` | integer | No | `30` | Timeout for bulk requests in seconds |
| `max_bulk_bytes` | integer | No | `5242880` | Maximum uncompressed bulk payload size in bytes |

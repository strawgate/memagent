---
title: "Inputs"
description: "The 8 input types logfwd supports — from file tailing to Arrow-native sources"
---

Sources produce raw bytes or Arrow batches independently and feed into a bounded pipeline channel. logfwd supports 8 input types across two modes:
...
Use logfwd as an OTLP relay or aggregator. Because the input produces structured Arrow data directly, there is no parsing overhead — just decode the protobuf envelope and convert. Resource attributes, trace context, and severity are all preserved.
...
**Key properties:** Profiles: `logs` (synthetic requests) and `record` (flat JSON from attributes). Zero external dependencies.

...
|-------|-------------|--------------|
| `IoError` | Transient I/O error (already exists) | `FailureAction::IoError` |
| `RetryAfter` | Server-requested backoff (already exists) | `FailureAction::RetryAfter` |
| `Reject` | Permanent rejection (already exists) | `FailureAction::Reject` |
| `Delay` | Slow delivery (already exists) | `FailureAction::Delay` |
| `Panic` | Sink panic (already exists) | `FailureAction::Panic` |
...
pub struct InvariantSet {
    /// Every input line must reach the sink at least once.
    ...
    /// Retry-exhausted/timed-out batches must not advance checkpoints.
...
impl InvariantSet {
    pub fn verify(&self, outcome: &TestOutcome) -> Result<(), Vec<String>> {
        ...
        if self.at_least_once_delivery {
            if outcome.delivered_lines < outcome.input_lines as u64 {
                failures.push(format!(
                    "at-least-once violation: delivered {} of {} lines",
                    outcome.delivered_lines, outcome.input_lines
                ));
            }
        }
...
|-----------|-------------|-------------|
| **ALO** | At-least-once: every input line eventually delivered | `delivered >= input_lines` |
| **CM** | Checkpoint monotonicity: offsets never decrease | `assert_monotonic(source_id)` |
| **OA** | Ordered ACK: checkpoint doesn't skip ahead | `BatchId` ordering in `PipelineMachine` |
| **SC** | Shutdown completeness: pipeline reaches Stopped | `sim.run()` returns `Ok` |
| **HTF** | Hold on transient failure: retry-exhausted batches don't advance checkpoint | `durable_offset == None` after all-fail |

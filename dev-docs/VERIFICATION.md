...
| `logfwd-io/otlp_receiver/projection.rs` | Experimental OTLP wire projection into Arrow (bypasses prost object graph) | 47 tests: prost-reference oracle parity (primitive, multi-resource/scope, randomized), malformed wire rejection (truncated varint/fixed64/fixed32/len, invalid UTF-8 in key/value/body, group mismatch, field zero, oversized field, wrong wire type), unsupported-but-valid AnyValue fallback (array/kvlist in body/attrs, mixed primitive+unsupported), ProjectedFallback mode parity. Proptest: 64-case single-container + 32-case multi-container randomized parity. |

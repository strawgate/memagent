#!/usr/bin/env python3
"""Generate the narrow OTAP fast-path encoder/decoder module.

This v1 generator is intentionally small and schema-driven rather than generic:
it validates the tiny OTAP envelope shape we own and emits a direct protobuf
encode/decode implementation for the current BatchArrowRecords / BatchStatus
messages.
"""
...
SPEC = ROOT / "crates/logfwd-output/codegen/otap_fast_v1.schema.json"
OUT = ROOT / "crates/logfwd-output/src/generated/otap_fast_v1.rs"


def main() -> int:
    spec = json.loads(SPEC.read_text())
    assert spec["schema"] == "otap-fast-v1"
    assert "BatchArrowRecords" in spec["messages"]
    assert "ArrowPayload" in spec["messages"]
    assert "BatchStatus" in spec["messages"]

    OUT.write_text(
        """// This file is @generated by scripts/generate_otap_fast_proto.py.
// Source spec: crates/logfwd-output/codegen/otap_fast_v1.schema.json
